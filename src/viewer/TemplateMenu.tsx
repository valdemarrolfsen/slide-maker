...
import { templates } from 'virtual:slide-maker/templates';
 ...
 * are the real template components rendered small, in the deck's own style:
 * the studio has already loaded that stylesheet, so a template mounted here
 * picks it up for free and the gallery restyles with the deck.
 */
export function TemplateMenu({ config, pending, error, onPick }: TemplateMenuProps) {
  ...
  const move = (delta: number) => {
    setCursor((current) => Math.max(0, Math.min(templates.length - 1, current + delta)));
  };
  ...
  return (
    <div className="sm-tmpl" ref={rootRef}>
      <button
        ...
        title={error || 'Append a slide built from a template'}
        aria-haspopup="menu"
        aria-expanded={open}
      >
        <Plus />
      ...
      {open && (
        <div className="sm-tmpl-panel" ref={panelRef} role="menu" aria-label="Slide templates">
              ...
              >
                ...
                >
                  <span
                    ...
                    aria-hidden="true"
                  >
                    <SlideFrame slide={template} total={1} config={config} />
                  </span>
                </span>
                <span className="sm-tmpl-name">
                  {template.label}
                  {template.source === 'local' && <i className="sm-tmpl-local">local</i>}
                </span>
              </button>
            ))}
          </div>
